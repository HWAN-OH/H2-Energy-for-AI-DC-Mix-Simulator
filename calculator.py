...
def calculate_integrated_tco(config, user_inputs):
    ...
    Now includes calculation for total serviceable users based on efficiency.
    """
    # --- 1. Unpack all inputs ---
    ...
    # --- 8. [UPDATED] Calculate Required Revenue and Business Metrics ---
    if target_irr > 0:
        pvaf = (1 - (1 + target_irr)**-5) / target_irr
    else:
        pvaf = 5
    required_annual_revenue = integrated_tco / pvaf if pvaf > 0 else 0
    
    # --- [NEW LOGIC] Calculate Serviceable Users ---
    # The physical DC size is based on the *un-adjusted* peak demand.
    # MirrorMind doesn't shrink the building, it makes the hardware inside more productive.
    physical_dc_mw = demand_profile['peak_demand_mw'].iloc[-1] 
    users_per_mw = ra.get('active_users_per_mw', 1)
    base_serviceable_users = physical_dc_mw * users_per_mw
    
    # If MirrorMind is applied, the same hardware can serve more users.
    total_serviceable_users = base_serviceable_users / workload_factor if workload_factor > 0 else 0
    ...
    summary = {
        ...
        "viability": {
            ...
            "total_serviceable_users": total_serviceable_users # 새로 추가된 결과값
        }
    }
    return summary
