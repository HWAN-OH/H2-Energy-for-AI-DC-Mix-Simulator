...
def calculate_business_case(dc_size_mw, use_clean_power, apply_mirrormind, high_perf_gpu_ratio, utilization_rate, paid_tier_fee, premium_tier_multiplier, lang):
    ...
    # --- 2. Calculate Hardware Mix, Max Capacity & Power ---
    ...
    max_power_kwh_consumption = total_dc_power_kw * HOURS_PER_YEAR

    # --- 3. Apply Real-World Constraints (Utilization) ---
    util_rate = utilization_rate / 100.0
    effective_token_capacity_m = max_token_capacity_m * util_rate
    actual_power_kwh_consumed = max_power_kwh_consumption * util_rate

    # --- 4. Calculate Demand, Revenue & Final P&L ---
    EFFICIENCY = biz_conf["intelligent_arch_efficiency"] if apply_mirrormind else 1.0
    final_token_capacity_m = effective_token_capacity_m * EFFICIENCY
    ...
    num_user_sets_supported = final_token_capacity_m / token_demand_per_user_set if token_demand_per_user_set > 0 else 0
    ...
    # --- 5. Build Complete P&L ---
    pnl = {}
    pnl['revenue'] = total_revenue
    
    power_cost_kwh = dc_conf["power_cost_kwh"][use_clean_power]
    annual_power_cost = actual_power_kwh_consumed * power_cost_kwh
    ...
    return {
        "assumptions": {
            ...
            "serviced_tokens_t": final_token_capacity_m / 1_000_000,
            "consumed_power_gwh": actual_power_kwh_consumed / 1_000_000
        },
        "pnl_annual": pnl,
