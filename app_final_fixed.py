...
if run:
    ...
    for idx, row in demand_df.iterrows():
        ...
        if phase == "Initial Phase (NG + Grid)":
            ...
            row_result.update({
                ...
                "Solar (MWh)": 0, "Wind (MWh)": 0, "H2 SOFC (MWh)": 0, "Diesel (MWh)": 0,
    ...
    # Format certain columns for readability
    formatted_df = result_df.copy()
    formatted_df["Total Cost (USD)"] = formatted_df["Total Cost (USD)"].apply(lambda x: f"${x:,.0f}")
    formatted_df["Emissions (ton)"] = formatted_df["Emissions (ton)"].apply(lambda x: f"{x:.1f}")
    formatted_df["Emission Factor (kg/kWh)"] = formatted_df["Emission Factor (kg/kWh)"].apply(lambda x: f"{x:.3f}")

    st.markdown("### 📊 Annual Results (2025–2029)")
